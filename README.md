...
独立仓库的 Windows 便携版通过 `v*` 标签发布：推送版本标签后，GitHub Actions 会构建 EXE、生成 SHA-256 校验文件，并附上示例配置和自动生成的发布说明。下载后可先校验 `SHA256SUMS.txt`，再将 EXE 放入博客根目录。该流程只生成发布产物，不会自动更新博客内嵌版本。
