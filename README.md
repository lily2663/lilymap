...
独立仓库的 Windows 便携版通过 `v*` 标签发布：推送版本标签后，GitHub Actions 先完成验证工作流，再构建 EXE、生成 SHA-256 校验文件，并附上示例配置和自动生成的发布说明。下载后可先校验 `SHA256SUMS.txt`，再将 EXE 放入博客根目录。该流程只生成发布产物，不会自动更新博客内嵌版本。

LilyMap 将 `lily-epitaph` 的兼容提交固定在 `.github/theme-compatibility.json`。每周检查上游新提交；发现变化时，先用完整验证工作流检查精确的新提交，通过后才开更新 pin 的 PR。这样上游主题不会悄悄漂移，也不会因旧 pin 永远冻结而漏掉新兼容性问题。
...
test/              domain, HTTP security, publishing and integration contracts
```
...
`npm run protocol:check` 会按主题发布的 v1 JSON Schema 校验固定主题提交中的内置模块 manifest、模板与资源路径；需先设置 `LILY_TEST_THEME_PATH` 指向主题 checkout。博客根目录的 `npm run check` 还会检查站点模块、Slot 兼容性、布局引用和配置值。卸载模块前必须先从所有布局中移除；卸载文件会进入 `.admin-trash/modules/`。协议约定见主题仓库的 [`docs/protocol/v1.md`](https://github.com/lily2663/lily-epitaph/blob/main/docs/protocol/v1.md)。
